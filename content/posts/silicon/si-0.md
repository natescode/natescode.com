...
If perfmance at all costs is the goal, then no GC for you!
...
WASM doesn't have GC yet so Rust is your best bet really, especiall now.
...
**SO THAT** _I can build robust full-stack websites and mobile application without Javascript_
...
Silicon primitive types
    ...
    opaque = type that hide internal implementation (WASM)

    int = LEB128 variable length integer. Maps to Javascript Number or BigInt (WASM)
...
[Part 2](/si-2) will cover implemetation tools.
